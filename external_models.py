...
class AxialAttention(nn.Module):
    ...
    def forward(self, x):
        # pdb.set_trace()
        if self.width:  # wenn width = TRUE dann stelle die Breite an die erste Stelle (nach batch)
            x = x.permute(0, 2, 1, 3)
        else:
            x = x.permute(0, 3, 1, 2)  # N, W, C, H
        N, W, C, H = x.shape
        x = x.contiguous().view(N * W, C, H)    # reshape x, sodass für jede Breite und jedes Batchelement ein Vektor entsteht 

        # Transformations
        qkv = self.bn_qkv(self.qkv_transform(x)) # ???
        q, k, v = torch.split(qkv.reshape(N * W, self.groups, self.group_planes * 2, H), [self.group_planes // 2, self.group_planes // 2, self.group_planes], dim=2)
        # splitte channels in Gruppen 

        # Calculate position embedding
        all_embeddings = torch.index_select(self.relative, 1, self.flatten_index).view(self.group_planes * 2, self.kernel_size, self.kernel_size)
        q_embedding, k_embedding, v_embedding = torch.split(all_embeddings, [self.group_planes // 2, self.group_planes // 2, self.group_planes], dim=0)
        # splitte pos. enc. in Gruppen
