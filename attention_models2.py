...
class ConditionalBatchNorm2d(nn.Module):
  def __init__(self, num_features, num_features_cond):
    super().__init__()
    self.num_features = num_features
    self.bn = nn.BatchNorm1d(num_features, affine=False)
    self.linear = nn.Linear(num_features_cond, num_features * 2)
  ...
  def forward(self, x, y):
    out = self.bn(x)
    tmp = self.linear(y.unsqueeze(0)).squeeze()
    gamma, beta = tmp[:self.num_features], tmp[self.num_features:]
...
class Attention_block(torch.nn.Module):
    ...
    def forward(self, data, slice_index = None, slice_max = None):
        ...
        x_att = self.attention(query, key, value, need_weights = False)[0].squeeze()
        x_att = self.layer_norm1(x_att+x_in)
...
class EncoderGlobal(torch.nn.Module):
    ...
    def forward(self,z_in, slice_index = None, slice_max = None):
        ...
        for block, skip, conv_z in zip(self.enc_blocks, self.conv_skip, self.conv_z):
            z_tmp = skip(z_in)
            z_in = block(z_in)
...
class Decoder(torch.nn.Module):
    def __init__(self, num_layer = 5, 
        ...
        if pos_encoding == True:
            if global_local == True:
                ...
                #self.layer_norm.append(ConditionalBatchNorm2d(num_nodes_first, num_feat_out))
        ...
        for i in range(num_layer-1):
            if i == 0:
                if skips:
                    ...
                    #self.layer_norm.append(ConditionalBatchNorm2d(num_nodes, num_feat_out))
    ...
    def forward(self, data, slice_index, slice_max):
        # data
        ...
        if self.global_local == True:
             #z_in, z_local, z_global_smoother = z_in  
             z_in, z_local = z_in  
        ...
        if self.spatial_feat == False or self.global_local == True:
            if self.masked_attention == False:
                ...
                for i, att_block in enumerate(self.att_blocks):
                    x_att.append(att_block([z_in[:, i*32 : (i+1)*32], xz], slice_index = slice_index, slice_max = slice_max))
        ...
        if self.global_local == True:
            z_in = torch.cat((z_slice, z_local), dim = 1)
...
class GlobalXY(torch.nn.Module):
    ...
    def forward(self, data, resolution = 128, z_resolution = None, slice_max = None, slice_index = None):
       ...
       ############## GLOBAL FEATURES #############
        ...
        #z_global_smoother = torch.mean(z_global, axis = 0).squeeze()

       ############## XY FEATURES #############
        z_xy = z_xy.unsqueeze(0)

        # make grid of points to interpolate
        x_tmp,y_tmp = torch.linspace(-1,1,resolution), torch.linspace(-1,1,resolution)
        x_tmp,y_tmp = torch.meshgrid(x_tmp,y_tmp, indexing = "ij")
        grid = torch.stack((y_tmp,x_tmp),2)
        grid = grid.unsqueeze(0).to(x_in.device)
        # interpolate grid points from z
        z_xy = grid_sample(z_xy, grid, mode = "bicubic", align_corners = False).squeeze()

        # make feature vector for each coordinate pair
        z_xy = z_xy.reshape(z_xy.shape[0], -1)
        z_xy = z_xy.moveaxis(0,1)
        ...
        x = self.dec([x_in,[z_global, z_xy]],  slice_index = slice_index, slice_max = slice_max)
